...
try
  ...
  %  javaaddpath([pwd '\loci_tools.jar']); %This is needed so the distributable app works.
    ...
    global c_settings c_original_settings;
...
try
    ...
    global image_file_path c_settings c_im c; %This retains the folder
    ...
    [v,image_file_path] = uigetfile(fullfile(image_file_path,'*.czi;*.tif;*.tiff'));
   
    if v ~= 0,
        open_image(v,1,c_settings.autofind, 1,0);
    end 
       catch ME    
   waitmsgbox(getReport(ME))
end
...
get_settings_from_gui();
save_analysis();
waitmsgbox ('Analysis saved. An iaf file for the associated image has been saved in the same folder, you can reload this to recover the cell layout.')
...
if get(findobj(c_hand.app, 'Tag','rename_mode'),'Value') == 1,
    ...
    [path, filename, ext] = fileparts(current);
    filename = fullfile(path, filename);
    
    new_name = rename_item(filename);
    if ~isempty(new_name),
       contents{hObject.Value} = [new_name ext]; 
...
function success = check_and_load(filename, force_png)
    success = save_if_dirty(1);
    if success == 1,
        success= open_analysis_or_image(filename, force_png,1); %We force display, dunno if we need to but safer
    end
...
contents = trim_file_roots(cellstr(list.String));
...
if (settings_old.channel ~= c_settings.channel),
    imstruct.image_ext = c.image_ext;
...
for filename = contents',
    i = i + 1;
    h = waitbar(i/count,h, fixstr(['Screenshotting ' filename{:}]));
    open_analysis_or_image(filename{:},0,1); %force jpg load, and display
    save_current_screenshot();
end
